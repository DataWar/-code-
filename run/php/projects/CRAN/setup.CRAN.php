...
$myCRAN = new myCRAN;
	$myCRAN->dataPath = "A:/project-CRAN/";
...
switch($a)
	...
	#  php -f run.php p=CRAN a=svn-CRAN
	# http://localhost/run/run.php?p=CRAN&a=svn-CRAN
	case "svn-CRAN":
	# C:\_git_\svn
		$myCRAN->summarizeSVN();
	break;
	
	#  php -f run.php p=CRAN a=summarize-CRAN l=sentencepiece
	# http://localhost/run/run.php?p=CRAN&a=summarize-CRAN&l=sentencepiece
	
	case "summarize-CRAN":
		$myCRAN->summarizeCRAN($l);
	break;
	
	
	
	
	#  php -f run.php p=CRAN a=build-sql-from-xls
	# http://localhost/run/run.php?p=CRAN&a=build-sql-from-xls
	case "build-sql-from-xls":
		$myCRAN->buildSQL();
	break;
	
	
	#  php -f run.php p=CRAN a=build-pipe-files
	case "build-pipe-files":
		$myCRAN->buildPipeFiles();
